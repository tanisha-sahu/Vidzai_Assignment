...
export default function QuizPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If quiz passed, refresh user context to update completedLevels
      if (res.data.passed) {
        toast.success(`🎉 You scored ${res.data.score} / ${res.data.total}`);
        // Refresh user data to show unlocked levels
        setTimeout(() => {
          if (refreshUser) {
            refreshUser();
          }
        ...
        toast.error(`You scored ${res.data.score} / ${res.data.total} - Try again!`);
      }
    } catch (err) {
      console.error(err);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
        <div className="text-center">
          <p className="text-slate-700 font-semibold mb-2">No quiz available for this level</p>
          <button onClick={() => navigate(`/levels/${id}`)} className="text-indigo-600 hover:underline">Back</button>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden">
        ...
        <div className="p-5 sm:p-6">
          ...
          <p className="text-xs sm:text-sm text-slate-500 mb-6">
            Choose the best answer for each question. You can&apos;t change options after submitting.
          </p>
